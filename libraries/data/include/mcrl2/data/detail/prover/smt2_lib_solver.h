...
//
/// \file mcrl2/data/detail/prover/smt2_lib_solver.h
/// \brief Abstract interface for SMT solvers based on the SMT-LIB v2 format

#ifndef MCRL2_DATA_DETAIL_PROVER_SMT2_LIB_SOLVER_H
#define MCRL2_DATA_DETAIL_PROVER_SMT2_LIB_SOLVER_H
...
namespace detail
{
/// The class SMT2_LIB_Solver is a base class for SMT solvers that read the SMT-LIB format
...
/// SMT_Solver.
///
/// The method SMT2_LIB_Solver::translate receives an expression of sort Bool in conjunctive normal form as parameter
/// a_formula and translates it to a benchmark in SMT-LIB format. The result is saved as field std::string f_benchmark.

class SMT2_LIB_Solver : public SMT_Solver
{
private:
  ...
  std::map<sort_expression, std::set<std::string>> f_sort_elements;
  ...
  void prettyPrintStringSet(const std::set<std::string>& inputSet) {
   mCRL2log(log::verbose)<< "{ ";
    for (auto it = inputSet.begin(); it != inputSet.end(); ++it) {
     mCRL2log(log::verbose) << "\"" << *it << "\"";
      // Add comma and space if not the last element
      if (std::next(it) != inputSet.end()) {
        mCRL2log(log::verbose) << ", ";
      }
    }
  ...
  void declare_sorts()
  {
    ...
    if (!f_sorts.empty())
    {
      ...
      for (std::map<sort_expression, std::size_t>::const_iterator i = f_sorts.begin(); i != f_sorts.end(); ++i)
      {
        ...
        //          {
        f_extrasorts = f_extrasorts + "(define-sort ";
        //          }

        v_sort = i->first;
        std::stringstream v_sort_string;
        v_sort_string << "sort" << i->second;
        f_extrasorts = f_extrasorts + v_sort_string.str() + " () (" + pp(v_sort);
        for (auto it = f_sort_elements[v_sort].begin(); it != f_sort_elements[v_sort].end(); ++it) {
          f_extrasorts  = f_extrasorts + " " + *it;
        }
        f_extrasorts = f_extrasorts + "))\n";
//        prettyPrintStringSet(f_sort_elements[v_sort]);
        mCRL2log(log::verbose) << "Formula to be solved: " << pp(v_sort) << " asdf " << pp(f_sort_elements[v_sort])
                               << std::endl;
      }
    }
  }
  ...
  void declare_operators()
  {
    ...
    if (!f_operators.empty())
    {
      ...
      for (std::map<function_symbol, std::size_t>::const_iterator i = f_operators.begin(); i != f_operators.end(); ++i)
      {
        ...
        f_operators_extrafuns = f_operators_extrafuns + "(declare-fun " + v_operator_string.str();
          ...
          for (auto v_sort_domain_elt : v_sort_domain_list)
          {
            if (is_function_sort(v_sort_domain_elt))
            {
              throw mcrl2::runtime_error(
                  "Function " + data::pp(i->first) + " cannot be translated to the SMT-LIB format.");
            }
  ...
  void declare_variables()
  {
    ...
    for (auto v_variable : f_variables)
    {
      ...
      {
        f_variables_extrafuns = f_variables_extrafuns + "(declare-const " + v_variable_string + " Real)";
      }
      else if (sort_int::is_int(v_sort))
      {
        f_variables_extrafuns = f_variables_extrafuns + "(declare-const " + v_variable_string + " Int)";
      }
      ...
      else
      {
        ...
        v_sort_string << "sort" << v_sort_number;
        f_variables_extrafuns
            = f_variables_extrafuns + "(declare-const " + v_variable_string + " " + v_sort_string.str() + ")";
      }
  ...
  void produce_notes_for_predicates()
  {
    ...
    if (f_bool2pred)
    {
      f_predicates_notes = "  :notes \"bool2pred was introduced, because the smt-lib format cannot deal\"\n"
  ...
  void translate_clause(const data_expression& a_clause, const bool a_expecting_predicate)
  {
    ...
    {
      throw mcrl2::runtime_error("Unable to handle the current clause (" + data::pp(a_clause) + ").");
    }
  }
  ...
  void translate_unknown_operator(const data_expression& a_clause)
  {
    ...
    std::size_t v_operator_number
        = f_operators.size();   // This is the value if v_operator does not occur in f_operators.
    ...
    auto v_op_sort = v_operator.sort();
    auto clause = pp(a_clause);
    std::map<sort_expression, std::set<std::string>>::const_iterator k
        = f_sort_elements.find(v_op_sort);

    mCRL2log(log::verbose) << " asdf " << v_operator <<clause
                           << std::endl;
    if (k == f_sort_elements.end()) // not found.
    {
      std::set<std::string> a;
      a.insert(clause);
      f_sort_elements[v_op_sort] = a;
    }
    else
    {
      f_sort_elements[v_op_sort].insert(clause);
    }
  ...
  void translate_true() { f_formula = f_formula + "true"; }

  void translate_false() { f_formula = f_formula + "false"; }

  void translate_function_symbol(const data_expression& a_clause)
  {
    f_formula = f_formula + pp(a_clause);
    //    return;
...
//    mCRL2log(log::verbose) << "Corresponding benchmark:" << std::endl << f_benchmark;
  ...
  {
    for (const variable& f_nat_variable : f_nat_variables)
    {
  ...
  {
    for (const variable& f_pos_variable : f_pos_variables)
    {
...
protected:
  ...
  void translate(data_expression_list a_formula)
  {
    ...
    f_benchmark = f_sorts_notes + f_operators_notes + f_predicates_notes + f_extrasorts + f_operators_extrafuns
                  + f_variables_extrafuns + f_extrapreds + f_formula + ")\n";
    mCRL2log(log::verbose) << "Corresponding benchmark:" << std::endl << f_benchmark;
  }
 ...
 *
 *  \code
 *  class cvc_smt_solver : public binary_smt2_solver< cvc_smt_solver > {
...
/// The class inherits from the class SMT2_LIB_Solver. It uses the SMT solver
...
///
/// The method z3_smt2_solver::is_satisfiable receives a formula in conjunctive normal form as parameter a_formula and
/// indicates whether or not this formula is satisfiable.
class z3_smt2_solver : public SMT2_LIB_Solver, public binary_smt2_solver<z3_smt2_solver>
{
  friend class binary_smt2_solver<z3_smt2_solver>;

private:

  inline static char const* name() { return "Z3"; }

  inline static void exec() { ::execlp("z3", "z3", "-smt2", "-in", (char*)nullptr); }
