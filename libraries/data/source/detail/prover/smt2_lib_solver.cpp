...
{
template < typename T >
bool binary_smt2_solver< T >::execute(std::string const& benchmark)
{
  int pipe_stdin[2];
  int pipe_stdout[2];
  int pipe_stderr[2];
  ...
  else
  {
    ...
    else
    {
      ...
      throw mcrl2::runtime_error(std::string("The SMT2 prover ") + T::name() + " does not work properly. " + message );
    }
 ...
 **/
template < typename T >
bool binary_smt2_solver< T >::usable()
{
  if (!binary_smt2_solver::execute("(declare-const x Int) (assert (> x 0)) (assert (< x 1)) (check-sat)"))
  {
    throw mcrl2::runtime_error(std::string("The SMT2 solver ") + T::name() + " is not available. \n" +
...
template class binary_smt2_solver< z3_smt2_solver >;
}
}
}
}
