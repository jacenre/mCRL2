...
#include "mcrl2/pbes/tools/pbescyclicpaths.h"
...
class pbescyclicpaths_tool: public pbes_input_tool<pbes_output_tool<pbes_rewriter_tool<rewriter_tool<input_output_tool>>>>
{
  protected:
    typedef pbes_input_tool<pbes_output_tool<pbes_rewriter_tool<rewriter_tool<input_output_tool>>>> super;

    pbescyclicpaths_options m_options;
  ...
  public:
    pbescyclicpaths_tool()
      : super(
        "pbescyclicpaths",
        "Jore Booy",
        "Simplify a pbes by checking if there are paths and cycles that do not affect the solution.",
    ...
    bool run() override
    {
      mCRL2log(verbose) << "pbescyclicpaths parameters:" << std::endl;
      mCRL2log(verbose) << "  input file:         " << m_input_filename << std::endl;
      mCRL2log(verbose) << "  output file:        " << m_output_filename << std::endl;

      m_options.rewrite_strategy = rewrite_strategy();
      pbescyclicpaths(input_filename(),
                  output_filename(),
                  pbes_input_format(),
                  pbes_output_format(),
                  m_options
...
{
  return pbescyclicpaths_tool().execute(argc, argv);
}
