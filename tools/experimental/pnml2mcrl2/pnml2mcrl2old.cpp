...
//==================================================
// pn2gsRetrieveTextWithCheck gets the contents of a child <text> element of cur, and checks it to format
// [a-zA-Z_][a-zA-Z0-9_]*
//==================================================
...
static ATermAppl pn2gsAterm_place(wxXmlNode* cur)
{
  ...
  while (cur)
  {
    ...
    else if (cur->GetName() == wxT("initialMarking"))
    {
      ...
      while (curl)
      {
        if (curl->GetName() == wxT("text"))
        {
          ...
          if (!Marking)
          {
            gsErrorMsg("Parsing of the initial marking for place %T failed\n", Aid);
            return NULL;
          }
        ...
        else if (with_colors && curl->GetName() == wxT("toolspecific"))
        {
          ...
          if (mcrl2marking != "")
          {
            ...
            if (!A)
            {
              gsErrorMsg("Parsing of the mCRL2 initial marking for place %T failed\n", Aid);
              return NULL;
            }
    ...
    else if (cur->GetName() == wxT("toolspecific"))
    {
      ...
      if (!sort.empty())
      {
        ...
        if (!Sort)
        {
          gsErrorMsg("Parsing of mCRL2 sort for place %T failed\n", Aid);
          return NULL;
        }
        Place_type = Sort;
      }
  ...
  return ATmakeAppl5(ATmakeAFun("place", 5, ATfalse),
    (ATerm)Aid,
    (ATerm)Aname,
    (ATerm)AinitialMarking,
    (ATerm)Place_type,
    (ATerm)Place_mcrl2initialMarking);
}
  ...
  while (cur)
  {
    ...
    else if (with_colors && cur->GetName() == wxT("toolspecific"))
    {
      ...
      if (!predicate.empty())
      {
        ...
        if (!Predicate)
        {
          gsErrorMsg("Parsing of the mCRL2 predicate for transition %T failed\n", Aid);
          return NULL;
        }
...
static ATermAppl pn2gsAterm_arc(wxXmlNode* cur)
{
  ...
  while (cur)
  {
    ...
    else if (cur->GetName() == wxT("name"))
    {
      ...
      if (!name.empty())
      {
        ...
        if (!Name)
        {
          gsErrorMsg("Parsing of the name of arc %T failed\n", Aid);
          return NULL;
        }
  ...
  return ATmakeAppl5(ATmakeAFun("arc", 5, ATfalse),
    (ATerm)Aid,
    (ATerm)Asource,
    (ATerm)Atarget,
    (ATerm)Atype,
...
static ATermAppl pn2gsAterm(wxXmlDocument& doc)
{
  ...
  while (cur)
  {
    ...
    else if (with_colors && cur->GetName() == wxT("toolspecific"))
    {
      ...
      if (!prelude.empty())
      {
        ...
        if (!Prelude)
        {
          gsErrorMsg("Parsing of the mCRL2 prelude failed\n");
          return NULL;
        }
        Prelude = type_check_data_spec(Prelude);
        if (!Prelude)
        {
          gsErrorMsg("Type-checking of the mCRL2 prelude failed\n");
          return NULL;
        }
        Net_prelude = Prelude;
      }
  ...
  return ATmakeAppl5(ATmakeAFun("PetriNet", 5, ATtrue),
    (ATerm)ATreverse(APlaces),
    (ATerm)ATreverse(ATransitions),
    (ATerm)ATreverse(AArcs),
    (ATerm)ANetID,
    (ATerm)Net_prelude);
}
...
static ATermList pn2gsGenerateActionsTrans()
{
  ...
  for (ATermList l = ATtableKeys(context.trans_name); !ATisEmpty(l); l = ATgetNext(l))
  {
    ...
    r = ATinsert(r,
      (ATerm)pn2gsGenerateAction_trans(ATgetAFun(e), ATgetAFun(ATtableGet(context.trans_name, (ATerm)e))));
  }
  return ATreverse(r);
}
...
static ATermList pn2gsGenerateActions(void)
{
  ...
  if (error)
    Ids = ATinsert(Ids, (ATerm)gsString2ATermAppl("error"));
...
static ATermAppl pn2gsSyncIn(ATermList Ids)
{
  ...
  {
    sync
      = gsMakeSync(pn2gsSyncIn(ATgetNext(Ids)), gsMakeParamId(ATmakeAppl0(ATgetAFun(ATgetFirst(Ids))), ATmakeList0()));
  }
  else
  {
...
static ATermAppl pn2gsSyncOut(ATermList Ids)
{
  ...
  if (ATgetLength(Ids) > 1)
  {
    sync = gsMakeSync(pn2gsSyncOut(ATgetNext(Ids)),
      gsMakeParamId(ATmakeAppl0(ATprependAFun("_", ATgetAFun(ATgetFirst(Ids)))), ATmakeList0()));
  }
  else
  {
    // ATgetLength(Ids) == 1, since the input-list is not-empty!
...
static ATermList pn2gsGenerateTransition(ATerm TransID)
{
  ...
  if (strcmp(CurrentTransName, "default_name"))
  {
    ...
    CurrentTransIn = ATmakeAppl0(
      ATprependAFun("T_", ATappendAFun(ATappendAFun(ATappendAFun(CurrentTransId, "_"), CurrentTransName), "_in")));
    CurrentTransOut = ATmakeAppl0(
      ATprependAFun("T_", ATappendAFun(ATappendAFun(ATappendAFun(CurrentTransId, "_"), CurrentTransName), "_out")));
  }
  else
  {
  ...
  if (!(ATtableGet(context.trans_in, TransID)) && !(ATtableGet(context.trans_out, TransID)))
  {
    ...
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentTrans, ATmakeList0()), ATmakeList0(), Process));
  ...
  else if (ATtableGet(context.trans_in, TransID) && !(ATtableGet(context.trans_out, TransID)))
  {
    ...
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentTransIn, ATmakeList0()), ATmakeList0(), Process));
    gsDebugMsg("Process: %T created.\n", CurrentTransIn);
  }
  else if (!(ATtableGet(context.trans_in, TransID)) && ATtableGet(context.trans_out, TransID))
  {
    ...
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentTransOut, ATmakeList0()), ATmakeList0(), Process));
    gsDebugMsg("Process: %T created.\n", CurrentTransOut);
  }
  else if (ATtableGet(context.trans_in, TransID) && ATtableGet(context.trans_out, TransID))
  {
    ...
    // create T_ti
    Process = gsMakeSync(
      gsMakeSync(gsMakeParamId(CurrentTransIn, ATmakeList0()), gsMakeParamId(CurrentTransOut, ATmakeList0())),
      gsMakeParamId(MonitorAction, ATmakeList0()));
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentTrans, ATmakeList0()), ATmakeList0(), Process));
    gsDebugMsg("Process: %T created.\n", CurrentTrans);
    // create T_ti_in
...
static ATermAppl pn2gsChoiceIn(ATermList Ids)
{
  ...
  if (ATgetLength(Ids) > 1)
  {
    choice = gsMakeChoice(gsMakeParamId(ATmakeAppl0(ATgetAFun(ATgetFirst(Ids))), ATmakeList0()),
      pn2gsChoiceIn(ATgetNext(Ids)));
  }
  else
  {
    // ATgetLength(Ids) == 1, since the input-list is not-empty!
...
static ATermAppl pn2gsChoiceOut(ATermList Ids)
{
  ...
  if (ATgetLength(Ids) > 1)
  {
    choice = gsMakeChoice(gsMakeParamId(ATmakeAppl0(ATprependAFun("_", ATgetAFun(ATgetFirst(Ids)))), ATmakeList0()),
      pn2gsChoiceOut(ATgetNext(Ids)));
  }
  else
  {
    // ATgetLength(Ids) == 1, since the input-list is not-empty!
...
static ATermList pn2gsGeneratePlace(ATerm PlaceID)
{
  ...
  if (strcmp(CurrentPlaceName, "default_name"))
  {
    ...
    CurrentPlaceAdd = ATmakeAppl0(
      ATprependAFun("P_", ATappendAFun(ATappendAFun(ATappendAFun(CurrentPlaceId, "_"), CurrentPlaceName), "_add")));
    CurrentPlaceIn = ATmakeAppl0(
      ATprependAFun("P_", ATappendAFun(ATappendAFun(ATappendAFun(CurrentPlaceId, "_"), CurrentPlaceName), "_in")));
    CurrentPlaceRem = ATmakeAppl0(
      ATprependAFun("P_", ATappendAFun(ATappendAFun(ATappendAFun(CurrentPlaceId, "_"), CurrentPlaceName), "_rem")));
    CurrentPlaceOut = ATmakeAppl0(
      ATprependAFun("P_", ATappendAFun(ATappendAFun(ATappendAFun(CurrentPlaceId, "_"), CurrentPlaceName), "_out")));
  }
  else
  {
    // name of the transition is "default_name"
    // name of the transition will not be used
    // make the processes: P_placeid     >-<     P_placeid_add     >-<     P_placeid_in     >-<     P_placeid_rem >-<
    // P_placeid_out
...

  {
    ...
    ATermAppl CondThen2;
    ATermAppl SumVar0
      = gsMakeDataVarId(ATmakeAppl0(ATmakeAFun("y", 0, ATtrue)), static_cast<ATermAppl>(mcrl2::data::sort_pos::pos()));
    ATermAppl SumVar1
      = gsMakeDataVarId(ATmakeAppl0(ATmakeAFun("z", 0, ATtrue)), static_cast<ATermAppl>(mcrl2::data::sort_pos::pos()));
    ATermList SumVars;
    ...
    CondThen0 = gsMakeSeq(gsMakeParamId(CurrentPlaceAdd, ATmakeList1((ATerm)SumVar0)),
      gsMakeParamId(CurrentPlace, ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpAdd, SumVar0, ProcVar))));
    ...
    CondThen1 = gsMakeSeq(gsMakeParamId(CurrentPlaceRem, ATmakeList1((ATerm)SumVar1)),
      gsMakeParamId(CurrentPlace,
        ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpMax, Number0, pn2gsMakeDataApplProd2(OpSubt, ProcVar, SumVar1)))));
    ...
    CondThen2 = gsMakeSeq(gsMakeSync(gsMakeParamId(CurrentPlaceAdd, ATmakeList1((ATerm)SumVar0)),
                            gsMakeParamId(CurrentPlaceRem, ATmakeList1((ATerm)SumVar1))),
      gsMakeParamId(CurrentPlace,
        ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpMax,
          Number0,
          pn2gsMakeDataApplProd2(OpSubt, pn2gsMakeDataApplProd2(OpAdd, SumVar0, ProcVar), SumVar1)))));
    ...
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(
        gsMakeProcVarId(CurrentPlace, ATmakeList1((ATerm) static_cast<ATermAppl>(mcrl2::data::sort_nat::nat()))),
        ATmakeList1((ATerm)ProcVar),
        Process));
    gsDebugMsg("Process: %T created.\n", CurrentPlace);
  }

  {
    ...
    ATermAppl ProcVar
      = gsMakeDataVarId(pn2gsPlaceParameter(CurrentPlaceAdd), static_cast<ATermAppl>(mcrl2::data::sort_pos::pos()));
    ATermAppl CondIf = pn2gsMakeDataApplProd2(OpGT, ProcVar, Number1);
    ATermAppl CondThen = gsMakeSync(gsMakeParamId(CurrentPlaceIn, ATmakeList0()),
      gsMakeParamId(CurrentPlaceAdd,
        ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpMax, Number1, pn2gsMakeDataApplProd2(OpSubt, ProcVar, Number1)))));
    ATermAppl CondElse = gsMakeParamId(CurrentPlaceIn, ATmakeList0());
    ATermAppl Process = gsMakeIfThenElse(CondIf, CondThen, CondElse);
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(
        gsMakeProcVarId(CurrentPlaceAdd, ATmakeList1((ATerm) static_cast<ATermAppl>(mcrl2::data::sort_pos::pos()))),
        ATmakeList1((ATerm)ProcVar),
        Process));
    gsDebugMsg("Process: %T created.\n", CurrentPlaceAdd);
  }

  {
    ...
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentPlaceIn, ATmakeList0()), ATmakeList0(), Process));
    gsDebugMsg("Process: %T created.\n", CurrentPlaceIn);
  }

  {
    ...
    ATermAppl ProcVar
      = gsMakeDataVarId(pn2gsPlaceParameter(CurrentPlaceRem), static_cast<ATermAppl>(mcrl2::data::sort_pos::pos()));
    ATermAppl CondIf = pn2gsMakeDataApplProd2(OpGT, ProcVar, Number1);
    ATermAppl CondThen = gsMakeSync(gsMakeParamId(CurrentPlaceOut, ATmakeList0()),
      gsMakeParamId(CurrentPlaceRem,
        ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpMax, Number1, pn2gsMakeDataApplProd2(OpSubt, ProcVar, Number1)))));
    ATermAppl CondElse = gsMakeParamId(CurrentPlaceOut, ATmakeList0());
    ATermAppl Process = gsMakeIfThenElse(CondIf, CondThen, CondElse);
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(
        gsMakeProcVarId(CurrentPlaceRem, ATmakeList1((ATerm) static_cast<ATermAppl>(mcrl2::data::sort_pos::pos()))),
        ATmakeList1((ATerm)ProcVar),
        Process));
    gsDebugMsg("Process: %T created.\n", CurrentPlaceRem);
  }

  {
    ...
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentPlaceOut, ATmakeList0()), ATmakeList0(), Process));
    gsDebugMsg("Process: %T created.\n", CurrentPlaceOut);
  }
...
static ATermList pn2gsCommList(ATermList List)
{
  ...
  {
    CurrentId = ATgetAFun(ATgetFirst(List));
    ATermAppl CommExpr = gsMakeCommExpr(
      gsMakeMultActName(ATmakeList2((ATerm)ATmakeAppl0(CurrentId), (ATerm)ATmakeAppl0(ATprependAFun("_", CurrentId)))),
      ATmakeAppl0(ATprependAFun("__", CurrentId)));
...
static ATermList pn2gsPlacesParameters(ATermList Places)
{
  ...
  while (ATisEmpty(Places) == ATfalse)
  {
    ...
    ATermAppl PlaceParameter
      = gsMakeDataVarId(pn2gsPlaceParameter(PlaceName), static_cast<ATermAppl>(mcrl2::data::sort_nat::nat()));
    // colored
...
static ATermAppl pn2gsMerge(ATermList Ids)
{
  ...
  if (ATgetLength(Ids) > 1)
  {
    merge = gsMakeMerge(gsMakeParamId(Id, ATmakeList1((ATerm)gsMakeId(pn2gsPlaceParameter(Id)))),
      pn2gsMerge(ATgetNext(Ids)));
  }
  else
  {
    // ATgetLength(Ids) == 1, since the input-list is not-empty!
...
static ATermList pn2gsInitialMarkings(ATermList Places)
{
  ...
  while (ATisEmpty(Places) == ATfalse)
  {
    ...
    if (!Marking)
      Marking = gsMakeId(gsString2ATermAppl("0"));
    ...
    if (Type && !ATisEqual(Type, Appl0))
    {
      ...
      if (!Marking || ATisEqual(Marking, Appl0))
        Marking = EmptyBag;
    }
...
static ATermList pn2gsGenerateProcEqns(ATermAppl NetID)
{
  ...
  //
  // P_pi_add(y:Pos) = block({"set of synchronisation of each incoming arc with itself"}, (y>1) -> (P_pi_in |
  // P_pi_add(max(1, y-1))) : P_pi_in)
  //
  // P_pi_in = "choice of the incoming arcs"
  //
  // P_pi_rem(z:Pos) = block({"set of synchronisation of each outgoing arc with itself"}, (z>1) -> (P_pi_out |
  // P_pi_rem(max(1, z-1))) : P_pi_out)
  //
  // P_pi_out = "choice of the outgoing arcs"

  // If there are no incoming or outgoing arcs, P_pi_add and P_pi_in respectively P_pi_add and P_pi_out are will never
  // be executed because of the fact that the maximum concurrency == 0. Since 'y' and 'z' are of type Pos, they must be
  // > 0 and thus execution of the named processes is prevented. Therefore, in that case it doesn't matter what P_pi_add
  // and P_pi_in respectively P_pi_add and P_pi_out look like. For ease of implementation, the parts in between " " will
  // be defined as tau.
  ...
  if (ATisEmpty(context.transitions) == ATtrue)
  {
    ...
    ProcessList = ATinsert(ProcessList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(gsString2ATermAppl("Trans"), ATmakeList0()), ATmakeList0(), gsMakeDelta()));
  }
  else
  {
    ...
    {
      ProcessList = ATinsert(ProcessList,
        (ATerm)gsMakeProcEqn(gsMakeProcVarId(gsString2ATermAppl("Trans"), ATmakeList0()),
          ATmakeList0(),
          gsMakeSeq(pn2gsChoiceIn(context.transitions), gsMakeParamId(gsString2ATermAppl("Trans"), ATmakeList0()))));
    }
    else
    {
      ProcessList = ATinsert(ProcessList,
        (ATerm)gsMakeProcEqn(gsMakeProcVarId(gsString2ATermAppl("Trans"), ATmakeList0()),
          ATmakeList0(),
          gsMakeSeq(pn2gsChoiceIn(ATinsert(context.transitions, (ATerm)gsString2ATermAppl("error"))),
    ...
    Process = gsMakeHide(pn2gsHideList(AllArcs),
      gsMakeBlock(pn2gsBlockList(AllArcs),
        gsMakeComm(pn2gsCommList(AllArcs),
          gsMakeMerge(gsMakeParamId(gsString2ATermAppl("Trans"), ATmakeList0()),
  ...
  else
  {
    ...
    for (ATermList Places = ATtableKeys(context.place_process_name); !ATisEmpty(Places); Places = ATgetNext(Places))
    {
      ...
      ATermList AssocArcs = ATmakeList0(); // in and out arcs of the place
      {
        ...
        if (MoreArcs)
          AssocArcs = ATconcat(AssocArcs, MoreArcs);
      ...
      if (!error)
      {
        Process = gsMakeHide(pn2gsHideList(AssocArcs),
          gsMakeBlock(pn2gsBlockList(AssocArcs), gsMakeComm(pn2gsCommList(AssocArcs), Process)));
      }
      else
      {
        Process = gsMakeHide(pn2gsHideList(AssocArcs),
          gsMakeBlock(ATinsert(pn2gsBlockList(AssocArcs), (ATerm)gsString2ATermAppl("_error")),
            gsMakeComm(pn2gsCommList(ATinsert(AssocArcs, (ATerm)gsString2ATermAppl("error"))), Process)));
      }
      // ***** end changed by Yarick.
    }
  ...
  ProcessList = ATinsert(ProcessList,
    (ATerm)gsMakeProcEqn(gsMakeProcVarId(gsString2ATermAppl("PetriNet"), pn2gsPlacesParameterTypes(Place_procs)),
  ...
  ProcessList
    = ATinsert(ProcessList, (ATerm)gsMakeProcEqn(gsMakeProcVarId(NetID, ATmakeList0()), ATmakeList0(), Process));
...
static ATermAppl pn2gsTranslate(ATermAppl Spec)
{
  ...
  if (ATisEqual(Prelude, Appl0))
    Prelude = mcrl2::data::detail::data_specification_to_aterm_data_spec(mcrl2::data::data_specification()); // NULL;
  ...
  while (ATisEmpty(APlaces) == ATfalse)
  {
    ...
    else
    {
      ...
      ATermAppl Type = type_check_data_expr(Value,
        static_cast<ATermAppl>(mcrl2::data::sort_nat::nat()),
        mcrl2::data::detail::data_specification_to_aterm_data_spec(mcrl2::data::data_specification()));
      if (!Type)
      {
        gsErrorMsg("Type-checking of the initial marking of place %T failed (%T is not a natural number)\n",
          CurrentKey,
          Value);
        return NULL;
      }
      ...
      if (!ATisEqual(Value, Appl0))
      {
        ...
        if (!Type)
        {
          gsErrorMsg("Type-checking of sort expression %P as a type of place %T failed \n", Value, CurrentKey);
          return NULL;
        }
      }
      ...
      if (!ATisEqual(Value, Appl0))
      {
        Type = type_check_data_expr(Value,
          static_cast<ATermAppl>(mcrl2::data::sort_bag::bag(mcrl2::data::sort_expression(SortValue))),
          Prelude);
        if (!Type)
        {
          gsErrorMsg("Type-checking of data expression %T as an initial mCRL2 marking of place %T failed \n",
            Value,
            CurrentKey);
          return NULL;
        }
    ...
    else
    {
      ...
      if (ATtableGet(context.place_name, (ATerm)CurrentSource) && ATtableGet(context.trans_name, (ATerm)CurrentTarget))
      {
        if (ATisEqual(CurrentType, Appl0))
        {
          ...
          ATtablePut(context.arc_out,
            (ATerm)CurrentKey,
            (ATerm)ATmakeAppl2(ATmakeAFun("arc_out", 2, ATfalse), (ATerm)CurrentSource, (ATerm)CurrentTarget));
        }
        else if (ATisEqual(CurrentType, gsString2ATermAppl("inhibitor")))
        {
          ...
          // key = id
          // value = arc_out(source, target)
          ATtablePut(context.arc_inhibit,
            (ATerm)CurrentKey,
            (ATerm)ATmakeAppl2(ATmakeAFun("arc_out", 2, ATfalse), (ATerm)CurrentSource, (ATerm)CurrentTarget));
        }
        else if (ATisEqual(CurrentType, gsString2ATermAppl("reset")))
        {
          ...
          gsWarningMsg("The reset arc %T going from place %T to transition %T is reversed (buggy pnml?)\n",
            ...
            CurrentSource,
            CurrentTarget);
          ATtablePut(context.arc_reset,
            (ATerm)CurrentKey,
            (ATerm)ATmakeAppl2(ATmakeAFun("arc_out", 2, ATfalse), (ATerm)CurrentSource, (ATerm)CurrentTarget));
        }
      }
      else if (ATtableGet(context.place_name, (ATerm)CurrentTarget)
               && ATtableGet(context.trans_name, (ATerm)CurrentSource))
      {
        if (ATisEqual(CurrentType, Appl0))
        {
          ...
          ATtablePut(context.arc_in,
            (ATerm)CurrentKey,
            (ATerm)ATmakeAppl2(ATmakeAFun("arc_in", 2, ATfalse), (ATerm)CurrentSource, (ATerm)CurrentTarget));
        }
        else if (ATisEqual(CurrentType, gsString2ATermAppl("inhibitor")))
        {
          ...
          ATtablePut(context.arc_inhibit,
            (ATerm)CurrentKey,
            (ATerm)ATmakeAppl2(ATmakeAFun("arc_in", 2, ATfalse), (ATerm)CurrentTarget, (ATerm)CurrentSource));
        }
        else if (ATisEqual(CurrentType, gsString2ATermAppl("reset")))
        {
      ...
      else
      {
        ...
        gsWarningMsg("The source or target of arc with id '%T' will not be translated, and thus this arc will not be "
                     "translated either.\n",
          CurrentKey);
      }
  ...
  gsDebugMsg(
    "  ID's of the two read-in arc tables. The first list contains the in-arcs, the second one the out-arcs. \n");
  ...
  while (ATisEmpty(Arcs) == ATfalse)
  {
    gsDebugMsg("Place '%T' has the following incoming arcs: %T\n",
      ATgetFirst(Arcs),
      ATtableGet(context.place_in, ATgetFirst(Arcs)));
    Arcs = ATgetNext(Arcs);
  }
  ...
  while (ATisEmpty(Arcs) == ATfalse)
  {
    gsDebugMsg("Transition '%T' has the following outgoing arcs: %T\n",
      ATgetFirst(Arcs),
      ATtableGet(context.trans_out, ATgetFirst(Arcs)));
    Arcs = ATgetNext(Arcs);
  }
  ...
  while (ATisEmpty(Arcs) == ATfalse)
  {
    gsDebugMsg("Place '%T' has the following outgoing arcs: %T\n",
      ATgetFirst(Arcs),
      ATtableGet(context.place_out, ATgetFirst(Arcs)));
    Arcs = ATgetNext(Arcs);
  }
  ...
  while (ATisEmpty(Arcs) == ATfalse)
  {
    gsDebugMsg("Transition '%T' has the following incoming arcs: %T\n",
      ATgetFirst(Arcs),
      ATtableGet(context.trans_in, ATgetFirst(Arcs)));
    Arcs = ATgetNext(Arcs);
  }
  ...
  while (ATisEmpty(Arcs) == ATfalse)
  {
    gsDebugMsg("Place '%T' has the following outgoing arcs: %T\n",
      ATgetFirst(Arcs),
      ATtableGet(context.place_inhibit, ATgetFirst(Arcs)));
    Arcs = ATgetNext(Arcs);
  }
  ...
  while (ATisEmpty(Arcs) == ATfalse)
  {
    gsDebugMsg("Place '%T' has the following outgoing arcs: %T\n",
      ATgetFirst(Arcs),
      ATtableGet(context.place_reset, ATgetFirst(Arcs)));
    Arcs = ATgetNext(Arcs);
  }
  ...
  for (ATermList Trans = ATtableKeys(context.trans_name); !ATisEmpty(Trans); Trans = ATgetNext(Trans))
  {
    ...
    if (!Arcs)
      Arcs = ATLtableGet(context.trans_out, (ATerm)Tran);
    else
    {
      ATermList Arcs2 = ATLtableGet(context.trans_out, (ATerm)Tran);
      if (Arcs2)
        Arcs = ATconcat(Arcs, Arcs2);
    }

    if (!Arcs)
      continue; // this transition has no ajusent arcs...
    ...
    for (; !ATisEmpty(Arcs); Arcs = ATgetNext(Arcs))
    {
      ...
      if (!ArcName || ATisEqual(ArcName, Appl0))
        continue;
    ...
    {
      ATermAppl Type
        = type_check_data_expr(Value, static_cast<ATermAppl>(mcrl2::data::sort_bool::bool_()), Prelude, Vars);
      if (!Type)
      {
        ATtableDestroy(Vars);
  ...
  if (!ATgetLength(ATtableKeys(context.arc_in)) && !ATgetLength(ATtableKeys(context.arc_out))
      && !ATgetLength(ATtableKeys(context.arc_inhibit)) && !ATgetLength(ATtableKeys(context.arc_reset)))
  {
  ...
  if (error)
  {
    ...
    ATermAppl ExtraEqn
      = gsMakeDataEqn(ATmakeList0(), sort_bool::true_(), nMaxTokens, gsMakeId(ATmakeAppl0(ATmakeAFunInt0(error))));
    ...
    ATermAppl NewDataEqnSpec
      = gsMakeDataEqnSpec(ATinsert(ATLgetArgument(ATAgetArgument(DataSpec, 3), 0), (ATerm)ExtraEqn));
...
static ATermList pn2gsGeneratePlaceAlternative(ATerm PlaceID)
{
  ...
  for (ATermList Lt = ATtableKeys(context.trans_name); !ATisEmpty(Lt); Lt = ATgetNext(Lt))
  {
    ...
    for (ATermList La = ATLtableGet(context.place_in, (ATerm)PlaceID); La && !ATisEmpty(La); La = ATgetNext(La))
    {
      ...
      if (!ATisEqual(ATAgetArgument(Arc, 1), PlaceID))
        continue;
      if (!ATisEqual(ATAgetArgument(Arc, 0), TransID))
        continue;
    ...
    for (ATermList La = ATLtableGet(context.place_out, (ATerm)PlaceID); La && !ATisEmpty(La); La = ATgetNext(La))
    {
      ...
      if (!ATisEqual(ATAgetArgument(Arc, 0), PlaceID))
        continue;
      if (!ATisEqual(ATAgetArgument(Arc, 1), TransID))
        continue;
      mult_c = ATinsert(mult_c, (ATerm)ArcID);
    }
    ...
    if (!nIn && !nOut && !inhib && !reset)
      continue;

    if (nOut > 0 && inhib)
    {
      gsWarningMsg(
        "Both output and inhibitor arcs connect place %T with transition %T. This transition can never fire.\n");
      continue;
    }
    ...
    if (inhib && reset)
      AR = ATappendAFun(CurrentPlaceId, "_arir_");
    else
    {
      if (inhib)
        AR = ATappendAFun(CurrentPlaceId, "_ari_");
      else if (reset)
        AR = ATappendAFun(CurrentPlaceId, "_arr_");
    }
    ...
    if (Type)
    {
      ...
      {
        ATermAppl Name = ATmakeAppl0(
          ATappendAFun(ATappendAFun(ATprependAFun("vp_", ATgetAFun(PlaceID)), "_"), ATgetName(ATmakeAFunInt0(i))));
    ...
    {
      if (d > 0)
        RightExpr = pn2gsMakeDataApplProd2(OpAdd, RightExpr, gsMakeId(ATmakeAppl0(ATmakeAFunInt0(d)))); // RightExpr=x+d;
      else if (d < 0)
        RightExpr = gsMakeDataAppl(OpInt2Nat,
          ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpSubt,
            RightExpr,
            gsMakeId(ATmakeAppl0(ATmakeAFunInt0(-d)))))); // RightExpr=Int2Nat(x-d);
    }
    else
    ...
    if (Type)
    {
      ...
      {
        if (nIn)
          RightExpr = pn2gsMakeDataApplProd2(OpAdd,
            RightExpr,
            pn2gsMakeBagVars(pn2gsMakeIds(ATgetSlice(VarNames, 0, nIn)))); // RightExpr+={|in|};
        if (nOut)
          RightExpr = pn2gsMakeDataApplProd2(OpSubt,
            RightExpr,
            pn2gsMakeBagVars(pn2gsMakeIds(ATgetTail(VarNames, static_cast<int>(nIn))))); // RightExpr-={|out|};
      }
      else
    ...
    if (inhib)
    {
      ...
      if (Cond)
        Cond = pn2gsMakeDataApplProd2(OpAnd, Cond, Cond1);
      else
        Cond = Cond1;
    }
    ...
    if (Type)
    {
      ...
      if (nOut > 0)
      {
        Cond = pn2gsMakeDataApplProd2(OpLTE,
          pn2gsMakeBagVars(pn2gsMakeIds(ATgetTail(VarNames, static_cast<int>(nIn)))),
          IdX); // subbag
      }
      if (inhib)
      {
    ...
    if (Body)
      Body = gsMakeChoice(Summand, Body);
    else
      Body = Summand;
    ...
    if (Type && nIn + nOut > 0)
    {
      ...
      for (size_t i = 0; i < nIn + nOut; i++) LeftType = ATinsert(LeftType, (ATerm)Type);
    }

    // extra equation
    EquationList = ATinsert(EquationList,
      (ATerm)gsMakeProcEqn(gsMakeProcVarId(LeftName, LeftType),
  ...
  //     ATermAppl
  //     VarX=gsMakeDataVarId(ATmakeAppl0(ATmakeAFunId("x")),static_cast<ATermAppl>(mcrl2::data::sort_nat::nat()));;
  ...
  // 	  int d=i-j;
  // 	  if(d>0)
  // RightExpr=pn2gsMakeDataApplProd2(OpAdd,RightExpr,gsMakeId(ATmakeAppl0(ATmakeAFunInt0(d))));//RightExpr=RightExpr+d;
  // 	  else if(d<0)
  // RightExpr=gsMakeDataAppl(OpInt2Nat,ATmakeList1((ATerm)pn2gsMakeDataApplProd2(OpSubt,RightExpr,gsMakeId(ATmakeAppl0(ATmakeAFunInt0(-d))))));//RightExpr=max(RightExpr-d,0);
  // 	}
  // 	ATermAppl Right=gsMakeParamId(CurrentPlace,ATmakeList1((ATerm)RightExpr));//make P_pi(max(x+i-j,0))
  ...
  // 	ATermAppl RightExpr=VarX;  //x;
  // 	if(i>0)
  // RightExpr=pn2gsMakeDataApplProd2(OpAdd,RightExpr,gsMakeId(ATmakeAppl0(ATmakeAFunInt0(i))));//RightExpr=RightExpr+i;
  ...
  else
  {
    ...
    ATermAppl ExtraSummand = gsMakeIfThenElse(ErrorCond,
      gsMakeSeq(ErrorAction, gsMakeParamId(CurrentPlace, ATmakeList1((ATerm)IdX))),
      gsMakeDelta());
    if (Body)
  ...
  ATermAppl VarX = gsMakeDataVarId(ATAgetArgument(IdX, 0),
    (Type) ? static_cast<ATermAppl>(mcrl2::data::sort_bag::bag(mcrl2::data::sort_expression(Type)))
           : static_cast<ATermAppl>(mcrl2::data::sort_nat::nat()));
  EquationList = ATinsert(EquationList,
    (ATerm)gsMakeProcEqn(gsMakeProcVarId(CurrentPlace,
                           ATmakeList1((ATerm)((Type) ? static_cast<ATermAppl>(mcrl2::data::sort_bag::bag(
                                                          mcrl2::data::sort_expression(Type)))
                                                      : static_cast<ATermAppl>(mcrl2::data::sort_nat::nat())))),
...
static ATermList pn2gsGetActionLists(size_t n, ATermList ActList)
{
  ...
  // cannot
  if (n > ATgetLength(ActList))
    return ATmakeList0();

  // one way
  if (n == ATgetLength(ActList))
    return ATmakeList1((ATerm)ActList);
...
static ATermList pn2gsGenerateTransitionAlternative(ATerm TransID)
{
  ...
  for (ATermList l = Arcs; !ATisEmpty(l); l = ATgetNext(l))
  {
    ...
    else
    {
      ...
      if (!VarName || ATisEqual(VarName, Appl0))
      {
        ...
        VarName = ATmakeAppl0(
          ATappendAFun(ATappendAFun(ATprependAFun("vt_", CurrentTransId), "_"), ATgetName(ATmakeAFunInt0(n))));
        n++;
      }
  ...
  Actions
    = ATinsert(Actions, (ATerm)ATmakeAppl0(ATappendAFun(ATprependAFun("t_", CurrentTransId), "_mon"))); // MonitorAction
  ParamList = ATinsert(ParamList, (ATerm)ATmakeList0());
...
{
  ATermList r = ATmakeList0();
  for (; !ATisEmpty(l); l = ATgetNext(l)) r = ATinsert(r, (ATerm)gsMakeId(ATAgetFirst(l)));
  return ATreverse(r);
}
...
static ATermList pn2gsMakeListOfLists(ATermList l)
{
  ...
  ATermList r = ATmakeList0();
  for (; !ATisEmpty(l); l = ATgetNext(l)) r = ATinsert(r, (ATerm)ATmakeList1(ATgetFirst(l)));
  return ATreverse(r);
}
...
static ATermList pn2gsMakeDataVarIds(ATermList l, ATermAppl Type)
{
  ...
  ATermList r = ATmakeList0();
  for (; !ATisEmpty(l); l = ATgetNext(l)) r = ATinsert(r, (ATerm)gsMakeDataVarId(ATAgetFirst(l), Type));
  return ATreverse(r);
}
